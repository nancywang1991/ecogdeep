...
for s, sbj in enumerate(sbj_ids):
    ...
    for t, time in enumerate(start_times):
        for itr in xrange(1):
            model_files = glob.glob('/home/wangnxr/models/ecog_vid_model_lstm_%s_itr_0_t_%i_*chkpt.h5' % (sbj, time))
            if len(model_files) == 0:
                continue
            model = load_model(model_files[0])
            layer_dict = dict([(layer.name, layer) for layer in model.layers[2].layers])
