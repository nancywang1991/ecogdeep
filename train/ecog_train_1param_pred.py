...
def f_nn(params):
    ...
    # Block 3
    x = Convolution2D(64, 2, 10, border_mode='same', name='block3_conv1')(x)
    #x = BatchNormalization(axis=1)(x)
    x = Activation('relu')(x)
    x = MaxPooling2D((2,2),  name='block3_pool')(x)

    # Block 4
    x = Convolution2D(128, 2, 10, border_mode='same', name='block4_conv1')(x)
    x = BatchNormalization(axis=1)(x)
    x = Activation('relu')(x)
    ...
    model.save("/home/wangnxr/model_ecog_1d_offset_15_%s.h5" % "_".join([str(param) for param in params]))
    pickle.dump(history_callback.history,open("/home/wangnxr/history_ecog_1d_offset_15_%s.p" % "_".join([str(param) for param in params]), "wb"))
