...
with open(result_table, 'wb') as csvfile:
    ...
    colors = 'gyc'
    rects_list = []
    for r, result_file in enumerate([svm_file, ecog_file]):
        ...
        for t in range(len(start_times)):
            ...
            rects.append(axes[2-t].bar(ind + width*(r*1.2), [score[t]*100 for score in average], width, color = colors[r], zorder=3))
    ...
    [axes[i].set_xticks(ind + 2*width / 2) for i in xrange(3)]
    plt.setp( axes[1].get_yticklabels(), visible=False)
    plt.setp( axes[2].get_yticklabels(), visible=False)
    axes[0].set_xticklabels(("S1", "S2", "S3", "S4"))
    axes[1].set_xticklabels(("S1", "S2", "S3", "S4"))
    axes[2].set_xticklabels(("S1", "S2", "S3", "S4"))

    #axes[2].text(0,100,"Pred_back", zorder=3)
    #axes[1].text(0,100,"Pred", zorder=3)
    #axes[0].text(0,100,"Decode", zorder=3)
    axes[0].set_title("Detect")
    axes[1].set_title("Pred")
    axes[2].set_title("Pred-b")
    lgd = axes[2].legend([rects_list[i, t][0] for i in xrange(2)], ("Baseline", "ECoG NN"))
    #    fig.tight_layout()
    fig.savefig("/home/wangnxr/results/tabulated_graph_ecog.png", bbox_extra_artists=(lgd,), bbox_inches='tight')
...
with open(result_table_valbest, 'wb') as csvfile:
    ...
    axes[2].text(3.5, 90, "Pred_back", zorder=3)
    axes[1].text(3.5, 90, "Pred", zorder=3)
    axes[0].text(3.5, 90, "Detect", zorder=3)
