...
"""Compiling results from ecog only and video only in order to get the sample average.


Example:
		$ python result_compilation.py

"""

with open("/home/wangnxr/results/ecog_vid_avg_summary_results_v3.txt", "wb") as summary_file:
    for s, sbj in enumerate(sbj_ids):
        for time in start_times:
            #Paths to best models
            model1 = glob.glob('/home/wangnxr/models/valbest/vid_model_lstm_%s_itr_*_t_%i_*' % (sbj, time))
            model2 = glob.glob('/home/wangnxr/models/valbest2/ecog_model_lstm20_%s_itr_*_t_%i_*' % (sbj, time))
            ...
            if len(model1) == 0 or len(model2)==0:
                continue
            #result files corresponding to the models
            ...
            # Average sigmoid scores from both models and select category
