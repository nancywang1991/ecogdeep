...
dest_dir = "/home/wangnxr/dataset/ecog_vid_combined_a0f_day6/"
ecog_files = glob.glob("/home/wangnxr/dataset/ecog_offset_0_a0f_day6/*/*/*")
#pdb.set_trace()
for ecog_file in ecog_files:
        #pdb.set_trace()
	sub_subfolder =  "%s/%s" % (dest_dir, "/".join(ecog_file.split(".")[0].split("/")[-3:-1]))
        subfolder =  "%s/%s" % (dest_dir, "/".join(ecog_file.split(".")[0].split("/")[-3]))
	...
	if not os.path.exists(sub_subfolder):
        ...
        vid_file = glob.glob("dataset/ecog_vid_combined_a0f/*/*/%s_*_%s.png" % (vid, frame))
        if len(vid_file)>0:
