...
space = {"conv_layer_units": hp.uniform("conv", 5, 15),
         "pool_layer": hp.uniform("pool", 1, 3),
         "conv_layer_filters": hp.choise("filters",[16, 32, 64, 128]),
...
def f_nn(params):
    ...
    # Block 2
    x = Convolution2D(64, 1, 10,  border_mode='same', name='block2_conv1')(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = MaxPooling2D((1,2),  name='block2_pool')(x)

    # Block 3
    x = Convolution2D(128, 1, 10, border_mode='same', name='block3_conv1')(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = MaxPooling2D((1,2),  name='block3_pool')(x)

    # Block 4
    x = Convolution2D(256, 1, 10, border_mode='same', name='block4_conv1')(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
