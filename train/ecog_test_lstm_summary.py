...
with open("/home/wangnxr/results/ecog_lstm_only_summary_results_v2.txt", "wb") as summary_writer:
    for s, sbj in enumerate(sbj_ids):
        for time in start_times:
            main_ecog_dir = '/home/wangnxr/dataset/ecog_vid_combined_%s_day%i/test/' % (sbj, days[s])
            for itr in xrange(3):
                model_files = glob.glob(
                    '/home/wangnxr/models/ecog_model_lstm_only_%s_itr_%i_t_%i_v2__weights_*.h5' % (sbj, itr, time))
		...
		validation_generator =  dgdx_val_edf
                ...
                with open("/home/wangnxr/results/%s_lstm_only.txt" % model_file.split("/")[-1].split(".")[0], "wb") as writer:
