...
def vid_model(weights=None):
    ...
    # Block 1
    x = TimeDistributed(Convolution2D(4, 2, 2, border_mode='same', subsample=(4,4)), name='block1_conv1')(input_tensor)
    # x = BatchNormalization(axis=1)(x)
    x = Activation('relu')(x)
    x = TimeDistributed(MaxPooling2D((3, 3)),name='block1_pool')(x)

    # Block 2
    x = TimeDistributed(Convolution2D(4, 2, 2, border_mode='same'), name='block2_conv1')(input_tensor)
    # x = BatchNormalization(axis=1)(x)
    x = Activation('relu')(x)
