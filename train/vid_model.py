...
def vid_model(weights=None):
    ...
    # Block 1
    x = Convolution2D(16, 5, 5, border_mode='same', subsample=(4,4), name='block1_conv1')(input_tensor)
    # x = BatchNormalization(axis=1)(x)
    x = Activation('relu')(x)
    x = MaxPooling2D((3, 3),name='block1_pool')(x)

    # Block 2
    x = Convolution2D(32, 3, 3, border_mode='same', name='block2_conv1')(x)
    # x = BatchNormalization(axis=1)(x)
    x = Activation('relu')(x)
    ...
    # x = Activation('relu')(x)
     # Block 3
    x = Convolution2D(64, 3, 3, border_mode='same', name='block3_conv1')(x)
     # x = BatchNormalization(axis=1)(x)
    x = Activation('relu')(x)
    #x = TimeDistributed(MaxPooling2D((3, 3)), name='block3_pool')(x)
    # Block 4
    x = Convolution2D(128, 3, 3, border_mode='same', name='block4_conv1')(x)
    # x = BatchNormalization(axis=1)(x)
    x = Activation('relu')(x)
    x = MaxPooling2D((3, 3), name='block4_pool')(x)
