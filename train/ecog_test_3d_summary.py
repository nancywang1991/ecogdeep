...
with open("/home/wangnxr/results/ecog_conv_3d_summary_results.txt", "wb") as summary_writer:
    for s, sbj in enumerate(sbj_ids):
        for time in start_times:
            main_ecog_dir = '/home/wangnxr/dataset/ecog_vid_combined_%s_day%i/test/' % (sbj, days[s])
            for itr in xrange(3):
                model_files = glob.glob(
                    '/home/wangnxr/models/ecog_model_3d_%s_itr_%i_t_%i__weights_*.h5' % (sbj, itr, time))
                ...
                test_datagen_edf = Ecog3DDataGenerator(
                    start_time=time,
                    center=True,
                ...
                with open("/home/wangnxr/results/%s_3d.txt" % model_file.split("/")[-1].split(".")[0], "wb") as writer:
