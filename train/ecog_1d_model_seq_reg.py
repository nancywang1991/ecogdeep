...
def ecog_1d_model(weights=None, channels=None):
    ...
    x = TimeDistributed(Convolution2D(32, 1, 3, border_mode='same'), name='block1_conv1')(x)
    #x = BatchNormalization(axis=1)(x)
    x = Activation('relu')(x)
    x = TimeDistributed(MaxPooling2D((1, 3)), name='block1_pool')(x)

    # Block 2
    x = TimeDistributed(Convolution2D(64, 1, 3, border_mode='same'), name='block2_conv1')(x)
    #x = BatchNormalization(axis=1)(x)
    x = Activation('relu')(x)
    x = TimeDistributed(MaxPooling2D((1, 3)), name='block2_pool')(x)
    # Block 3
    x = TimeDistributed(Convolution2D(128, 1, 3, border_mode='same'), name='block3_conv1')(x)
    #x = BatchNormalization(axis=1)(x)
    x = Activation('relu')(x)
    ...
    x = Dropout(0.1)(x)
    x = TimeDistributed(Dense(128, W_regularizer=l2(0.01)), name='fc1')(x)
